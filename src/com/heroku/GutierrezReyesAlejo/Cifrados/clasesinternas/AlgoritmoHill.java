...
public class AlgoritmoHill {
	...
	public String performance(String temp, int s) {
		cofact(km, s);
		String acum = "";
		while (temp.length() > s) {
	...
	public String encripta(String line, String key) {
		choice = 1;
		double sq = Math.sqrt(key.length());
		int size = (int) sq;
		if (sq != (long) sq)
			throw new IllegalArgumentException("Se necesita una llave con numeros cuadrados. Ej. 2x2 3x3 4x4");
			//return "Cannot Form a square matrix";
		else if (check(key, size)) {
			return performance(line, size);
		}
		return "";
	}

	public String desencripta(String line, String key) {
		choice = 2;
		double sq = Math.sqrt(key.length());
		int size = (int) sq;
		if (sq != (long) sq)
			throw new IllegalArgumentException("Se necesita una llave de descifrado con numeros cuadrados. Ej. 2x2 3x3 4x4");
			//return "Cannot Form a square matrix";
		else if (check(key, size)) {
			return performance(line, size);
		}
